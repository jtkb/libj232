...
// ########################################################################
JNIEXPORT jint JNICALL
Java_com_javatechnics_rs232_Serial_nativeTestOpen  (JNIEnv *env, jobject obj){
    fd = open("/dev/ttyS0", O_RDONLY | O_NOCTTY);
    if (fd == -1) throw_ioexception(env, errno);
    return fd;
}

JNIEXPORT jint JNICALL
Java_com_javatechnics_rs232_Serial_nativeTestOpen__Ljava_lang_String_2 (JNIEnv *env,
                                                                        jobject obj,
                                                                        jstring fileDescriptor,
                                                                        int flags){
    int return_value = -1;
    int native_flags = get_real_flags(java_open_flags, open_flags, \
                                            flags, number_open_flags);
    const jbyte *c_path = (*env)->GetStringUTFChars(env, fileDescriptor, NULL);
    if (c_path == NULL){
        //fd = open(c_path, O_RDONLY | O_NOCTTY);
        fd = open(c_path, native_flags);
        if (fd == -1) throw_ioexception(env, errno);
        return_value = fd;
        (*env)->ReleaseStringUTFChars(env, fileDescriptor, c_path);
    }
 ...
 */
JNIEXPORT jint JNICALL 
Java_com_javatechnics_rs232_Serial_nativePreparePort  (JNIEnv * env,
                                                        jobject jobj,
                                                        jint fd){
    int return_value = 0;
    ...
    return_value = tcsetattr(fd,TCSANOW,&newtio);
    if (return_value != -1){
        mcr = 0;	
